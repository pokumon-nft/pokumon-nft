...
async function main() {
  const namesFile = fs.readFileSync("./ai_models/aimon_names.txt", "utf8");
  ...
  for (let id = 1; id < loopCount; id++) {
    ...
    const description =
      'Aimon is the only unique digital asset in the world generated by AI and developed for the dApp called "Aimon" These monsters live in https://monsters.ai/, and you can raise them there or have them fight each other.';
    ...
    await firebase.addNFT(
      "https://storage.googleapis.com/aimon/" + id + ".png",
      name,
      id,
      tokenURI
    );
